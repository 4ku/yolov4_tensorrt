...
def build_engine(onnx_path, cfg_file_path, model_name, category_num, do_int8, dla_core, verbose=False):
    ...
    with trt.Builder(TRT_LOGGER) as builder, builder.create_network(*EXPLICIT_BATCH) as network, trt.OnnxParser(network, TRT_LOGGER) as parser:
        ...
        network = add_yolo_plugins(
            network, cfg_file_path, model_name, category_num, TRT_LOGGER)
...
def onnx_to_tensorrt(onnx_file, config_file, data_file, int8, dla_core, verbose):
    """Create a TensorRT engine for ONNX-based YOLO."""

    with open(data_file, 'r') as file:
        category_num = len(file.readlines())
    model_name = onnx_file.split('/')[-1].split('.')[0]

    engine = build_engine(onnx_file, config_file, model_name, 
            category_num, int8, dla_core, verbose)
    if engine is None:
...
if __name__ == '__main__':
    ...
    parser.add_argument("--onnx_file", default="yolov4.onnx",
    ...
    onnx_to_tensorrt(args.onnx_file, args.config_file, args.data_file, args.int8, args.dla_core, args.verbose)
