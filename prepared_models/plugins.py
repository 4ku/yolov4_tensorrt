...
try:
    ...
    ctypes.cdll.LoadLibrary(os.path.join(cur_dir, "..", "trt_yolo", "yolo_layer", "libyolo_layer.so"))
except OSError as e:
...
def add_yolo_plugins(network, cfg_file_path, model_name, num_classes, logger):
    """Add yolo plugins into a TensorRT network."""
    input_width, input_height = get_input_wh(cfg_file_path)
    yolo_whs = get_yolo_whs(model_name, input_width, input_height)
    if not verify_classes(cfg_file_path, num_classes):
