...
def yolo_to_onnx(weights, config_file, data_file):
    ...
    output_file_path = model_name + '.onnx' 

    if not verify_classes(config_file, category_num):
    ...
    layer_configs = parser.parse_cfg_file(config_file)
    ...
    if 'yolov3' in model_name:
        ...
        elif 'yolov4x-mish' in model_name:
        ...
        raise SystemExit('ERROR: unknown model (%s)!' % model_name)

    # Create a GraphBuilderONNX object with the specified output tensor
    # dimensions.
    print('Building ONNX graph...')
    builder = GraphBuilderONNX(model_name, output_tensor_dims)
...
if __name__ == '__main__':
    ...
    yolo_to_onnx(args.weights, args.config_file, args.data_file)