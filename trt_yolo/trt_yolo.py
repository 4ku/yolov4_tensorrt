...
class TrtYOLO(object):
    ...
    def __init__(self, config_file='prepared_models/yolov4.cfg', weights='prepared_models/yolov4.trt', 
                data_file='prepared_models/coco.names', letter_box=False, cuda_ctx=None):
        """Initialize TensorRT plugins, engine and conetxt."""
        cur_dir = os.path.dirname(os.path.realpath(__file__))
        config_file =  os.path.join(cur_dir, '..', config_file)
        weights =  os.path.join(cur_dir, '..', weights)
        data_file =  os.path.join(cur_dir, '..', data_file)
        ...
        self.engine = self._load_engine(weights)
        ...
        self.vis = BBoxVisualization(data_file)
        config = configparser.ConfigParser(strict=False)
        config.read(config_file)
        width = int(config['net']['width'])
        height = int(config['net']['height'])
        self.input_shape = (height, width)

    def _load_engine(self, weights):
    ...
    def draw_bboxes(self, img, boxes, confs, clss):
        return self.vis.draw_bboxes(img, boxes, confs, clss)
